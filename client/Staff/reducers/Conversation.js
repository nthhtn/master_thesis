...
export default function (state = initialState, action) {
	switch (action.type) {
		...
		case 'UPDATE_CONVERSATION': return { ...state, list: state.list.map((item) => item._id === action.id ? { ...item, ...action.data } : item) };
		case 'DELETE_CONVERSATION': return { ...state, list: state.list.filter((item) => item._id !== action.id) };
