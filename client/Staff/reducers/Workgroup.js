...
export default function (state = initialState, action) {
	switch (action.type) {
		...
		case 'UPDATE_WORKGROUP': return { ...state, list: state.list.map((item) => item.id === action.id ? { ...item, ...action.data } : item) };
		case 'DELETE_WORKGROUP': return { ...state, list: state.list.filter((item) => item.id !== action.id) };
